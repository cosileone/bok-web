    ...
    <div className="flex">
      {[...Array(5).keys()].map((index) => (
        <StarIcon
          ...
          className={clsx(
            "h-5 w-5",
            rating > index ? "fill-blue-600" : "fill-neutral-300",
          )}
        />
      ))}
    </div>
...
function Review({
  ...
  return (
    <figure
      className={clsx(
        "animate-fade-in rounded-3xl bg-white p-6 opacity-0 shadow-md shadow-neutral-900/5",
        className,
      )}
    ...
    >
      ...
      <figcaption className="mt-3 text-sm text-neutral-600 before:content-['–_']">
        {author}
      </figcaption>
    </figure>
  );
...
function ReviewGrid() {
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 h-32 bg-gradient-to-b from-neutral-50" />
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-neutral-50" />
    </div>
  );
}
